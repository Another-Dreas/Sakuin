...
function EditGoalContent() {
  ...
  const { updateGoal, goals, loadGoals } = useGoalStore();
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Update status if currentAmount now exceeds targetAmount
      const goal = goals.find(g => g.id === id);
      if (goal) {
        const status = goal.currentAmount >= data.targetAmount ? 'completed' : 'active';
  ...
  if (!isLoaded && id) {
    return <div className="flex justify-center items-center h-screen"><div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin" /></div>;
  }
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      <header className="flex-none flex items-center space-x-3 p-4 pt-8 pb-4 border-b border-border shadow-sm z-20 bg-background relative">
        <button type="button" onClick={() => router.back()} className="p-2 -ml-2 rounded-full hover:bg-muted transition-colors">
        ...
        <CardContent className="p-5">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="Contoh: Laptop Gaming" className="h-12 rounded-xl" {...register("name")} />
